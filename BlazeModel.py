...
class BlazePose(nn.Module):
    def __init__(self, num_keypoints: int):
        ...
        self.conv10_ = nn.Upsample(scale_factor=2, mode='bilinear',align_corners=True)
        # up sample layer
        ...
        self.conv12a = BlazeBlock(32, 96, 2)
        self.conv13a = BlazeBlock(96, 192, 2)
        self.conv14a = BlazeBlock(192, 288, 2)
        ...
        self.conv14b = nn.Sequential(
            nn.Conv2d(192, 288, 3, 1, 1, bias=False),
            nn.BatchNorm2d(288),
            h_swish()
            )

        self.conv15 = nn.Sequential(
            BlazeBlock(288, 288, 2),
            BlazeBlock(288, 288, 2)
        )
        self.conv16 = nn.Sequential(
            nn.AdaptiveAvgPool2d(output_size=(1, 1)),
            nn.Flatten(1),
            nn.Linear(288, self.num_keypoints*2)
        )
    ...
    def forward(self, x):
        ...
        # print(y0.shape)
        # print(self.conv10_(y0).shape)
        # print(self.upsample0(x1).shape)
        ...
        # return heatmap
        z3 = self.conv12a(x0) + self.conv12b(y1)
        # shape = (1, 32, 32, 96)
        z2 = self.conv13a(z3) + self.conv13b(y2)
        # # shape = (1, 16, 16, 192)
        z1 = self.conv14a(z2) + self.conv14b(y3)
        # # shape = (1, 8, 8, 288)
        z0 = self.conv15(z1)
        # # shape = (1, 2, 2, 288)
        joints = self.conv16(z0)
        # print(joints.shape)
        result = [heatmap, joints]
        return result[train_mode]
...
if __name__ == "__main__":
    model = BlazePose(14)
    torch.save(model, "BlazePoseTest.model")
